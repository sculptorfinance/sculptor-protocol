...
describe("ICO Locked", function () {
  ...
  before(async () => {
    ...
    //  await incentive.start();
     ...
     const ICOLOCK = await ethers.getContractFactory("ICOLock");
     icoLock = await ICOLOCK.deploy(
      multifeedistributor.address,
      maxMintLockedTeam
     );

     // setMinter function for give permission to incentive contract that can mint token
     await multifeedistributor.setMinters([incentive.address, icoLock.address]); //, icoLock.address
    ...
    // console.log(223, gFTM.toString(), gDAI.toString());
  ...
  it("Deposit, Borrow, Repay and Withdraw", async () => {
    ...
    // icoLock send to multifee
    await icoLock.deposit(
      [user3.address, user4.address, user5.address],
      ["5000000000000000000000000","7000000000000000000000000","8000000000000000000000000"]
    );
    const lockedBalances3 = await multifeedistributor.lockedBalances(user3.address);
    expect(lockedBalances3.total.toString()).to.equal("5000000000000000000000000");
    ...
    // await network.provider.send("evm_mine");
    await sculptorToken.connect(user2).approve(multifeedistributor.address, "5550000000000000000000");
    await multifeedistributor.connect(user2).stake("550000000000000000000", false);

    // increase block timestamp
    await network.provider.send("evm_increaseTime", [86400*90]);
    await network.provider.send("evm_mine");

    // stake token [locked]
    await sculptorToken.connect(user2).approve(multifeedistributor.address, "5550000000000000000000");
    await multifeedistributor.connect(user2).stake("550000000000000000000", true);
    await multifeedistributor.connect(user2).withdraw("550000000000000000000");
    // await multifeedistributor.connect(user2).getReward();
    ...
    const earn3 = await multifeedistributor.earnedBalances(user3.address);
    console.log(364, earn3.toString());
