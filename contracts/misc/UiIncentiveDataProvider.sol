...
contract UiIncentiveDataProvider is IUiIncentiveDataProvider {
  ...
  function test()
    ...
    returns (address)
  {
    address x = chefIncentive.poolConfigurator();
    return x;
  }
  ...
  function test2()
    ...
    returns (address)
  {
    address x = chefIncentive.registeredTokens(0);
    return x;
  }

  function test2(address _token)
    external
    view
    returns (uint256)
  {
    (,,uint256 time,,) = chefIncentive.poolInfo(_token);
    /* uint256 time = x.lastRewardTime; */
    return time;
  }
